...
export default function SettingsPanel({
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-bot-card border border-bot-border rounded-xl w-[600px] max-h-[80vh] flex flex-col shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-bot-border">
        ...
        {/* Tabs */}
        <div className="flex border-b border-bot-border">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              ...
              className={`flex items-center gap-2 px-5 py-3 text-sm font-medium transition-colors border-b-2 ${
                activeTab === tab.id
                  ...
                  : "border-transparent text-bot-muted hover:text-white"
              }`}
            >
          ...
          {activeTab === "trading" && (
            <>
              ...
              <div className="space-y-1">
                ...
                />
                <div className="flex justify-between text-[10px] text-bot-gray">
                  <span>1h</span>
                  <span>12h</span>
                  <span>24h (1d)</span>
                  <span>48h (2d)</span>
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left px-4 py-3 rounded-lg border transition-all ${
                          selected
                            ? "bg-bot-cyan/15 border-bot-cyan/50 ring-1 ring-bot-cyan/30"
                            : "bg-bot-bg border-bot-border hover:border-bot-cyan/30 hover:bg-bot-bg/80"
                        }`}
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <span className={`text-sm font-semibold ${selected ? "text-bot-cyan" : "text-white"}`}>
                              {m.name}
                            </span>
                            <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${
                              selected ? "bg-bot-cyan/20 text-bot-cyan" : "bg-gray-700 text-gray-400"
                            }`}>
                              {m.tag}
                            </span>
                          </div>
        ...
        {/* Footer */}
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-bot-border">
          <button
            onClick={onClose}
...
function SectionTitle({
  ...
  return (
    <div className="mb-2">
      ...
      <div className="text-xs text-bot-muted">{subtitle}</div>
    </div>
  );
}
    ...
    <div className="space-y-1">
      ...
      <div className="relative">
        <input
          ...
          className="w-full bg-bot-bg border border-bot-border rounded-lg px-3 py-2.5 pr-10 text-sm text-white placeholder-bot-gray focus:outline-none focus:border-bot-cyan/50 transition-colors font-mono"
        />
        <button
          onClick={onToggle}
